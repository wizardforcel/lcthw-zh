...
执行`break`的更好方式，是提供`file.c line`的格式，便于你去诶那个定位到了正确的地方。我在这个`break`中这样做。
...
+ 你可以开启OS上的“核心转储”，当程序崩溃时你会得到一个核心文件。这个核心文件就像是对程序的验尸，便于你了解崩溃时发生了什么，以及由什么原因导致。修改`ex31.c`使它在几个迭代之后崩溃，之后尝试得到它的核心转储并分析。
