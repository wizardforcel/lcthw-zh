...
既然你已经知道这些文件做什么了，下面是我的shell会话，用于构建`libex29.so和`ex29`并随后运行它。下面的代码中你可以学到如何手动构建：
...
> 有时候你会通常运行`cc -Wall -g -DNDEBUG -ldl ex29.c -o ex29`，并且认为它能够正常工作，但是没有。在一些平台上，库的顺序会影响到它是否生效，这也没什么理由。在Debian或者Ubuntu中你需要执行`cc -Wall -g -DNDEBUG ex29.c -ldl -o ex29`。它是唯一的方式，所以虽然我在这里使用了OSX，但是以后如果你链接动态库的时候它找不到某个函数，要试着自己解决问题。
...
+ 你注意到我在`libex29.c`中写的的不良代码了吗？我使用了一个`for`循环来检查`'\0'`的结构，修改它们使这些函数总是接收字符串长度，并在函数内部使用。
