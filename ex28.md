...
接下来我使用`install`程序来创建`lib`目标的目录。其中我通过使用两个为安装者提供便利的变量，尝试让安装尽可能灵活。`DESTDIR`交给安装者，便于在安全或者特定的目录里执行自己的构建。`PREFIX`在别人想要将项目安装到其它目录而不是`/usr/local`时会被使用。

Makefile:47

在此之后我使用`install`来实际安装这个库，到它需要安装的地方。
